...
namespace ransac_node{
    ...
    Wall RansacNode::Line2Wall(std::vector<Point> data,Line ab,bool TB=false){
        double a=ab.a;
        double b=ab.b;
        Wall best_wall;
    ...
    std::vector<RansacNode::Line> RansacNode::ransacs(std::vector<Point> data){
        std::vector<Line> lines;
        for(int i=0;i<max_lines;i++){
            Line line=ransac(data);
    ...
    void RansacNode::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        ...
        // get front wall
        std::vector<Wall> ransacs(scan_points);
